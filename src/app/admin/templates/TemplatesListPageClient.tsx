...
interface TemplatesListPageClientProps {
    initialTemplates: StoryTemplate[];
}

const TemplatesListPageClient: React.FC<TemplatesListPageClientProps> = ({ initialTemplates }) => {
    ...
    const fetchTemplates = async (filterValue: string) => {
        try {
            setLoading(true);
            const response = await storyService.getTemplates(filterValue || undefined);
...
export default TemplatesListPageClient;
